...
const KEYCHAIN_VERSION_OPTIONS = [
  ...
  },
  {
    ...
    description:
      'CubeX Wallet-local 9.2.3 wrapper with the Android prompt patch.',
  },
  {
    key: '10.0.0',
    label: '10.0.0',
...
export default function DevDataKeychain(): JSX.Element {
  ...
  const handleShareDebugInfo = useCallback(async () => {
    try {
      ...
      const fileName = `cubex-keychain-debug-${dayjs().format(
        'YYYYMMDD-HHmmss',
      )}.json`;
      ...
      const result = await shareLocalFile({
        ...
        subject: fileName,
        message: 'CubeX Wallet keychain debug info',
      });
